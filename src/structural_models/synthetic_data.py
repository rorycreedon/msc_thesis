...
class SimpleSCM:
    ...
    def simulate_data(self) -> None:
        ...
        self.scm.add_variable(name="X1", distribution=norm, loc=30, scale=5)

        # Unobserved variable U
        self.scm.add_variable(name="U", distribution=norm, loc=20, scale=5)

        # X1 and U cause X2
        self.scm.add_relationship(
            causes={"X1": 0.5, "U": 1}, effect="X2", noise_dist=norm, loc=0, scale=0
        )

        # X1 and X2 causes Y
        self.scm.add_binary_outcome(
            name="Y_true", weights={"X1": 1, "X2": 1}, noise_dist=norm, loc=-60, scale=5
        )
    ...
    def classify_data(self) -> Union[pd.Series, pd.DataFrame, LogisticRegression]:
        ...
        clf = LogisticRegression(random_state=0).fit(
            self.scm.data[["X1", "X2"]], self.scm.data["Y_true"]
        )
        y_pred = clf.predict(self.scm.data[["X1", "X2"]])
        X_neg = self.scm.data.loc[y_pred == 0, ["X1", "X2"]]
    ...
    def gen_weighted_adjacency_matrix(self) -> np.ndarray:
        ...
        :return: W_adjacency
        """
        return self.scm.generate_adjacency_matrix(["X1", "X2"])
