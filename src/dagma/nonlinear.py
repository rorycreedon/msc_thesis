...
class DagmaNonlinear:
    ...
    def minimize(
        ...
        if mask is None:
            mask = torch.ones_like(self.model.fc1.weight)
        self.vprint(f"\nMinimize s={s} -- lr={lr}")
        ...
        for i in range(max_iter):
            ...
            # PRUNE GRADIENTS
            self.model.fc1.weight.grad = self.model.fc1.weight.grad * mask
    ...
    def fit(
        ...
        if mask is not None:
            mask = mask.T.repeat_interleave(self.model.dims[1], dim=0)
