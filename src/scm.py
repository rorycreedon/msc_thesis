...
class StructuralCausalModel:
    ...
    def append_data(self, df: pd.DataFrame, ids: pd.Series):
        ...
        cols_to_fill = [col for col in self.data.columns if ((col not in df.columns) and (col != 'ID') and (col != 'recourse_eligible'))]
        ...
        for col in cols_to_fill:
            ...
            if col in self.distributions:
                ...
                self.data[col].iloc[-N_append:] = self.data.loc[self.data['ID'].isin(ids.values)][col].values
                ...
                logit_p = np.sum(
                    [weight * self.data[var].iloc[-N_append:] for var, weight in self.outcomes[col]['weights'].items()], axis=0) + noise
                p = expit(logit_p)
