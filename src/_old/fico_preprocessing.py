...
class Data_Cleaner:
    ...
    def __predict_feature_weighted(
        self, row, good_data_masked, no_neighbours, orig_array, ft_idx
    ):
    ...
    def __remove_row_with_vals_np(self, data, target, vals):
        # Convert data and target to numpy arrays first, if they aren't already
        data = np.array(data)
        target = np.array(target)
    
        # Find the rows where any element is in vals
        rows_to_remove = np.any(np.isin(data, vals), axis=1)
    
        # Select the rows to keep and the rows to remove
        removed_data = data[rows_to_remove]
        removed_target = target[rows_to_remove]
    
        data = data[~rows_to_remove]
        target = target[~rows_to_remove]
    ...
    def __process_and_predict(
        self, all_data, target_col, target_val, exclude=None, model="linear"
    ):
        # -- Split data --
        ...
        if exclude != None or exclude == []:
            ...
            X_pred = self.__remove_col_with_vals(
                X_target, exclude
            )  # The x used to predict
    ...
    def remove_8(self, kNN, prediction_type):
        ...
        X_good, hold1, hold2, hold3 = self.__remove_row_with_vals_np(
            self.X, self.y, [-7, -8, -9]
        )
            ...
            for row in range(self.num_samples):
                if self.X[row][fix_col] == -8:
                    ...
                    if prediction_type == "mean":
                        imputed = self.__predict_feature_mean(
                            row_to_comp, X_good_masked, kNN, X_good_scaled, fix_col
                        )

                    elif prediction_type == "weighted":
                        imputed = self.__predict_feature_weighted(
                            row_to_comp, X_good_masked, kNN, X_good_scaled, fix_col
                        )

                    X_no_8[row][fix_col] = (
                        imputed * scaler.scale_[fix_col] + scaler.mean_[fix_col]
                    )
